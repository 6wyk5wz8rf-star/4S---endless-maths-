...
function makeScaffold({ strand, values, hint, steps, model, visual, alternatives }) {
  return {
    hint: hint ?? "Look for a fact or structure you already know.",
    visual: visual ?? defaultVisual(strand, values),
...
const BUILD1_QUESTION_FAMILIES = [
  ...
  define({
    ...
    generate({ rng, target, challenge = target }) {
      ...
      return question(this, {
        difficulty: useThousands ? 39 : 32, instruction: "What number is this?", display: useThousands ? `${thousands} thousands + ${hundreds} hundreds + ${tens} tens + ${ones} ones` : `${hundreds} hundreds + ${tens} tens + ${ones} ones`,
...
function modeMatches(family, mode) {
  ...
  if (mode === "think") return modes.includes("think") || Number(family.thinking) >= 2;
...
function tailoredModel(item) {
  ...
  if (match) {
    const addend = Number(match[1]);
    const total = Number(match[2]);
    const nearby = addend > 1 ? addend - 1 : Math.min(total - 1, addend + 2);
    return { title: "Watch a nearby missing part", display: `${formatNumber(nearby)} + □ = ${formatNumber(total)}`, lines: [`${formatNumber(total)} − ${formatNumber(nearby)} = ${formatNumber(total - nearby)}`], answer: String(total - nearby) };
  }
  match = compactDisplay.match(/^□\s*\+\s*(\d+)\s*=\s*(\d+)$/);
  if (match) {
    const addend = Number(match[1]);
    const total = Number(match[2]);
    const nearby = addend > 1 ? addend - 1 : Math.min(total - 1, addend + 2);
  ...
  match = compactDisplay.match(/^(\d+)\s*×\s*□\s*=\s*(\d+)$/);
  if (match) {
    const factor = Number(match[1]);
    const missing = Math.max(2, answer - 1);
    return { title: "Watch a nearby missing factor", display: `${formatNumber(factor)} × □ = ${formatNumber(factor * missing)}`, lines: [`${formatNumber(factor * missing)} ÷ ${formatNumber(factor)} = ${formatNumber(missing)}`], answer: String(missing) };
  }
  match = compactDisplay.match(/^□\s*×\s*(\d+)\s*=\s*(\d+)$/);
  if (match) {
    const factor = Number(match[1]);
  ...
  if (match) {
    const divisor = Number(match[1]);
    const quotient = Math.max(1, Number(match[2]) - 1);
    return { title: "Watch a nearby missing dividend", display: `□ ÷ ${formatNumber(divisor)} = ${formatNumber(quotient)}`, lines: [`${formatNumber(divisor)} × ${formatNumber(quotient)} = ${formatNumber(divisor * quotient)}`], answer: String(divisor * quotient) };
  }
  match = compactDisplay.match(/^(\d+)\s*÷\s*□\s*=\s*(\d+)$/);
  if (match) {
    const quotient = Number(match[2]);
    const divisor = Math.max(2, answer - 1);
    return { title: "Watch a nearby missing divisor", display: `${formatNumber(quotient * divisor)} ÷ □ = ${formatNumber(quotient)}`, lines: [`${formatNumber(quotient * divisor)} ÷ ${formatNumber(divisor)} = ${formatNumber(quotient)}`], answer: String(divisor) };
  }
  if (compactDisplay.includes("□") && Number.isFinite(values.numerator) && Number.isFinite(values.denominator)) {
    const unitMatch = compactDisplay.match(/=\s*(\d+)$/);
    if (unitMatch) {
      const shownAmount = Number(unitMatch[1]);
      const numerator = Number(values.numerator);
      const denominator = Number(values.denominator);
      const unit = shownAmount / numerator;
      const nearbyUnit = Math.max(1, unit - 1);
      return { title: "Watch a nearby missing whole", display: `${fractionToken(numerator, denominator)} of □ = ${formatNumber(nearbyUnit * numerator)}`, lines: [`One part is ${formatNumber(nearbyUnit)}.`, `${formatNumber(nearbyUnit)} × ${denominator} = ${formatNumber(nearbyUnit * denominator)}`], answer: String(nearbyUnit * denominator) };
    }
  }
  match = compactDisplay.match(/^(\d+)\s*−\s*(\d+)$/);
  if (match && Number(match[1]) - Number(match[2]) === answer) {
    const minuend = Number(match[1]) + 1;
    const subtrahend = Number(match[2]) + 1;
    return { title: "Watch a nearby subtraction", display: `${formatNumber(minuend)} − ${formatNumber(subtrahend)}`, lines: [`The same amount was added to both numbers.`, `${formatNumber(minuend)} − ${formatNumber(subtrahend)} = ${formatNumber(answer)}`], answer: String(answer) };
  }
  if (item.strand === "addition" && Number.isFinite(a) && Number.isFinite(b) && answer === a + b && !item.display.includes("□")) {
    const shift = a >= 1000 ? 100 : a >= 100 ? 10 : 1;
    const relatedA = a > 3 ? a - shift : a + shift;
    const relatedB = a <= 3 && b > 1 ? b - shift : b;
    const broadPart = relatedB >= 10 ? Math.floor(relatedB / 10) * 10 : relatedB;
    const remainder = relatedB - broadPart;
    const lines = [`${formatNumber(relatedA)} + ${formatNumber(broadPart)} = ${formatNumber(relatedA + broadPart)}`];
    if (remainder) lines.push(`${formatNumber(relatedA + broadPart)} + ${formatNumber(remainder)} = ${formatNumber(relatedA + relatedB)}`);
    return { title: "Watch a nearby addition", display: `${formatNumber(relatedA)} + ${formatNumber(relatedB)}`, lines, answer: String(relatedA + relatedB) };
  }
  if (item.strand === "subtraction" && Number.isFinite(a) && Number.isFinite(b) && answer === a - b && !item.display.includes("□")) {
    const shift = a >= 1000 ? 100 : a >= 100 ? 10 : 1;
    const relatedA = a + shift;
    const relatedB = b + shift;
    const broadPart = relatedB >= 10 ? Math.floor(relatedB / 10) * 10 : relatedB;
    const remainder = relatedB - broadPart;
    const lines = [`${formatNumber(relatedA)} − ${formatNumber(broadPart)} = ${formatNumber(relatedA - broadPart)}`];
    if (remainder) lines.push(`${formatNumber(relatedA - broadPart)} − ${formatNumber(remainder)} = ${formatNumber(relatedA - relatedB)}`);
    return { title: "Watch a nearby subtraction", display: `${formatNumber(relatedA)} − ${formatNumber(relatedB)}`, lines, answer: String(relatedA - relatedB) };
  }
  if (item.strand === "multiplication" && Number.isFinite(a) && Number.isFinite(b) && answer === a * b && !item.display.includes("□")) {
    const relatedA = a > 3 ? a - 1 : a + 1;
    const tens = Math.floor(relatedA / 10) * 10;
    const ones = relatedA - tens;
    const lines = tens
      ? [`${formatNumber(tens)} × ${formatNumber(b)} = ${formatNumber(tens * b)}`, `${formatNumber(ones)} × ${formatNumber(b)} = ${formatNumber(ones * b)}`, `${formatNumber(tens * b)} + ${formatNumber(ones * b)} = ${formatNumber(relatedA * b)}`]
      : [`${formatNumber(relatedA)} groups of ${formatNumber(b)} make ${formatNumber(relatedA * b)}`];
    return { title: "Watch a nearby product", display: `${formatNumber(relatedA)} × ${formatNumber(b)}`, lines, answer: String(relatedA * b) };
  }
  if (item.strand === "division" && Number.isFinite(a) && Number.isFinite(b) && b > 0 && answer === a / b && !item.display.includes("□")) {
    const relatedQuotient = Math.max(1, answer - 1);
    const relatedDividend = relatedQuotient * b;
    return {
      title: "Watch a nearby quotient",
      display: `${formatNumber(relatedDividend)} ÷ ${formatNumber(b)}`,
      lines: [`${formatNumber(b)} × ${formatNumber(relatedQuotient)} = ${formatNumber(relatedDividend)}`, `So ${formatNumber(relatedDividend)} ÷ ${formatNumber(b)} = ${formatNumber(relatedQuotient)}`],
      answer: String(relatedQuotient),
    };
  }
  if (compactDisplay.includes("of") && Number.isFinite(a) && Number.isFinite(values.numerator) && Number.isFinite(values.denominator)) {
    const numerator = Number(values.numerator);
    const denominator = Number(values.denominator);
    const relatedWhole = a - denominator * 2;
    const relatedAnswer = relatedWhole * numerator / denominator;
    if (relatedWhole > 0 && Number.isInteger(relatedAnswer)) {
      return {
        title: "Watch a nearby fraction",
        display: `${fractionToken(numerator, denominator)} of ${formatNumber(relatedWhole)}`,
        lines: [`${formatNumber(relatedWhole)} ÷ ${denominator} = ${formatNumber(relatedWhole / denominator)}`, `${formatNumber(relatedWhole / denominator)} × ${numerator} = ${formatNumber(relatedAnswer)}`],
        answer: String(relatedAnswer),
      };
    }
  }
...
function contextualModel(item) {
  ...
  if (visual.kind === "relationship") {
    return {
      ...
      display: `${visual.left ?? "Known"} ${visual.connector ?? "helps"} ${visual.right ?? "new"}`,
  ...
  if (visual.kind === "worked-example") {
    ...
    return { title: "Watch one line at a time", display: lines[0] ?? item.display, lines: lines.slice(1), answer: item.answer };
  }
  if (visual.kind === "number-line" || visual.kind === "bead-string") {
    return {
      title: "Watch the intervals",
      display: `${formatNumber(visual.min ?? 0)} — ${formatNumber(visual.max ?? 1)}`,
      lines: [item.scaffold.hint],
      answer: item.answer,
    };
  }
...
export class FluencyEngine {
  constructor({ seed = `session-${Date.now()}`, challenge = 52, focus = null, mode = "mix", recentSignatures = [], learningState = {} } = {}) {
    this.seed = String(seed);
    this.rng = mulberry32(hashSeed(this.seed));
    this.challenge = clamp(Number(challenge), 0, 100);
    this.focus = focus;
    this.mode = ["mix", "focus", "quick", "think", "my-mix"].includes(mode) ? mode : "mix";
  ...
  getAdaptation() {
    return { performanceBias: this.performanceBias, mode: this.mode, scheduled: this.scheduled.length };
  }
  ...
  recordResponse({ item, question: legacyQuestion, correct = false, firstTry = false, supportUsed = 0, modelUsed = false, responseMs = null, misconception = null } = {}) {
    ...
    this.performanceBias = clamp(secure >= 6 ? 4 : secure >= 4 ? 2 : errors >= 4 ? -4 : errors >= 2 ? -2 : 0, -5, 5);
  ...
  generateNearTransfer(answered) {
    ...
    for (let attempt = 0; attempt < 60; attempt += 1) {
      ...
      const valid = validateQuestion(candidate).valid && questionSignature(candidate) !== questionSignature(answered) && itemMatchesFocus(candidate, this.focus);
      if (!valid) continue;
      fallback ??= candidate;
  ...
  takeScheduled() {
    ...
    for (const entry of due) {
      ...
      if (validation.valid && (!repeated || intentionalRepeat) && itemMatchesFocus(entry.item, this.focus) && Math.abs(entry.item.difficulty - this.challenge) <= 28) return entry.item;
    }
    return null;
  }
  ...
  finishItem(item, target, familyId = item.family) {
    ...
    const model = tailoredModel(item);
    item.scaffold.model = model === item.scaffold.model ? contextualModel(item) : model;
    ...
    if (!item.metadata.designedConnection && item.connections?.length && this.scheduled.length < 12) {
      const offsets = [0, 3, 8];
  ...
  next() {
    ...
    const target = rhythmTarget(this.rng, this.challenge, this.performanceBias, this.mode);
    const candidates = QUESTION_FAMILIES.filter((family) => {
      ...
      return inRange && contentFits && focusMatches(family, this.focus) && modeMatches(family, this.mode);
    });
    const source = candidates.length ? candidates : QUESTION_FAMILIES.filter((family) => focusMatches(family, this.focus) && modeMatches(family, this.mode));
    const safeSource = source.length ? source : QUESTION_FAMILIES.filter((family) => modeMatches(family, this.mode));
    const weighted = source.map((family) => {
      ...
      const learning = learningForFamily(family, this.learningState);
      ...
      const retrievalDue = learning && (questionsSince > 20 || ageDays > 3) ? 1.55 : 1;
    ...
    for (let attempt = 0; attempt < 80; attempt += 1) {
      ...
      if (validation.valid && itemMatchesFocus(item, this.focus) && difficultyFits && rhythmFits && !this.recentSignatures.includes(signature)) {
