...
const visual = {
  ...
  array: (rows, columns, title = `${rows} rows of ${columns}`) => rows * columns <= 96 && rows <= 12 && columns <= 12
    ? ({ kind: "array", title, rows, columns })
    : ({ kind: "multiplication-rectangle", title: "Partition the array", factor: columns, parts: [Math.floor(rows / 10) * 10, rows % 10].filter(Boolean) }),
  counters: (groups, perGroup, title = `${groups} equal groups`) => groups <= 12 && perGroup <= 12
    ? ({ kind: "counters", title, groups, perGroup })
    : ({ kind: "relationship", title: "Use equal groups", left: `${groups} groups of ${perGroup}`, right: `${groups} × ${perGroup} = ${groups * perGroup}`, connector: "means" }),
  groups: (total, groupSize, title = `Group ${total} in ${groupSize}s`) => total % groupSize === 0 && total / groupSize <= 12
    ? ({ kind: "groups", title, total, groupSize })
    : ({ kind: "relationship", title: "Use the inverse fact", left: `${groupSize} × ${Math.floor(total / groupSize)}${total % groupSize ? ` + ${total % groupSize}` : ""} = ${total}`, right: `${total} ÷ ${groupSize}`, connector: "checks" }),
...
export const BUILD2_FAMILIES = [
  define({
    ...
    generate({ rng, target, challenge = target }) {
      ...
      const common = {
        ...
        visual: boundary <= 20 ? visual.tenFrame(a, boundary, `Make ${boundary}`) : visual.partWhole(boundary, [a, complement]),
  ...
  define({
    ...
    generate({ rng, target, challenge = target }) {
      ...
      if (structure === "missing-groups") return makeQuestion(this, { ...common, display: `□ groups of ${a} make ${product}`, answer: b, hint: `How many ${a}s make ${product}?`, visual: visual.relationship(`□ × ${a}`, product, "makes") });
      if (structure === "missing-group-size") return makeQuestion(this, { ...common, display: `${b} equal groups make ${product}. Each group has □`, answer: a, hint: `Use ${product} ÷ ${b}.`, visual: visual.relationship(`${b} × □`, product, "makes") });
  ...
  define({
    ...
    generate({ rng, target, challenge = target }) {
      ...
      if (structure === "missing-number-groups") return makeQuestion(this, { ...common, display: `□ ÷ ${divisor} = ${quotient}`, answer: dividend, hint: "Undo division with multiplication.", visual: visual.relationship(`${divisor} × ${quotient}`, "the missing whole", "makes") });
      return makeQuestion(this, { ...common, display: `${dividend} ÷ □ = ${quotient}`, answer: divisor, hint: `What multiplied by ${quotient} makes ${dividend}?`, visual: visual.relationship(`${quotient} × □`, dividend, "makes") });
    },
  }),
  define({
  ...
  define({
    ...
    generate({ rng, target }) {
      ...
      const dividend = divisor * quotient;
      const firstQuotient = quotient % 10 === 0 ? quotient - 10 : Math.floor(quotient / 10) * 10;
      const secondQuotient = quotient - firstQuotient;
      const scale = quotient % 10 === 0 ? 10 : 2;
      const knownQuotient = quotient / scale;
      const knownDividend = divisor * knownQuotient;
      const common = { difficulty: clamp(target, 52, 93), values: { a: dividend, b: divisor }, structure, operation: "division", visual: visual.partWhole(dividend, [divisor * firstQuotient, divisor * secondQuotient], "Partition the dividend") };
      if (structure === "divide-best-known-fact") {
        const helpfulFact = `${knownDividend} ÷ ${divisor} = ${knownQuotient}`;
        return makeQuestion(this, {
          ...common, type: "choice", instruction: `Which fact helps most with ${dividend} ÷ ${divisor}?`, display: `${dividend} ÷ ${divisor}`,
          answer: helpfulFact, choices: choices(rng, helpfulFact, [`${dividend} ÷ ${scale} = ${dividend / scale}`, `${divisor} + ${divisor}`, `${quotient} × ${scale}`]),
          hint: "Choose an exact whole-number fact with the same divisor.",
        });
      }
      const hints = { "divide-inverse": `Think ${divisor} × □ = ${dividend}.`, "divide-partition": `Split ${dividend} into ${divisor * firstQuotient} and ${divisor * secondQuotient}.`, "divide-halving": divisor === 4 || divisor === 8 ? "Halve in stages." : "Use an inverse fact.", "divide-scale": `Use ${knownDividend} ÷ ${divisor} = ${knownQuotient}, then scale by ${scale}.` };
      const steps = structure === "divide-partition"
        ? [`${divisor * firstQuotient} ÷ ${divisor} = ${firstQuotient}`, `${divisor * secondQuotient} ÷ ${divisor} = ${secondQuotient}`, `${firstQuotient} + ${secondQuotient} = ${quotient}`]
        : [`Find a known multiple of ${divisor}.`, "Scale or partition to reach the dividend."];
      return makeQuestion(this, { ...common, display: `${dividend} ÷ ${divisor}`, answer: quotient, hint: hints[structure] ?? `Use ${divisor} × ${quotient} = ${dividend}.`, steps });
    },
  }),
  define({
    ...
    generate({ rng, target }) {
      ...
      if (structure === "digit-value") {
        const placeName = ["thousands", "hundreds", "tens", "ones"][placeIndex];
        return makeQuestion(this, { ...common, instruction: `What is the value of the ${placeName} digit ${digits[placeIndex]}?`, display: format(value), answer: digitValue, hint: `Find the ${placeName} place.` });
      }
  ...
  define({
    ...
    generate({ rng, target }) {
      ...
      let left; let right; let leftValue; let rightValue; let questionValues = { a, b };
      ...
      else if (structure === "compare-near-round-product") { const round = Math.ceil(a / 10) * 10; const near = round - 1; left = `${near} × ${b}`; right = `${round} × ${b - 1}`; leftValue = near * b; rightValue = round * (b - 1); questionValues = { a: near, b, round }; }
      ...
      const answer = leftValue === rightValue ? "Equal" : leftValue > rightValue ? left : right;
      return makeQuestion(this, { difficulty: clamp(target, 70, 98), type: "choice", instruction: "Which is larger?", display: "Compare the structure", answer, choices: shuffle(rng, [left, right, "Equal"]), values: questionValues, structure, hint: "Notice which factor changed and by how much.", visual: visual.relationship(left, right, "compare") });
    },
  }),
  define({
  ...
  define({
    ...
    generate({ rng, target }) {
      ...
      let display; let result; let hint; let steps; let questionAnswer = answer;
      let questionValues = { a: factor, b: answer, factor, offset };
      ...
      else if (structure === "missing-multiply-subtract") {
        const product = factor * answer;
        const usesSubtraction = product - offset >= 0;
        result = usesSubtraction ? product - offset : product + offset;
        display = usesSubtraction ? `${factor} × □ − ${offset} = ${result}` : `${factor} × □ + ${offset} = ${result}`;
        hint = "Undo the final operation first.";
        steps = [usesSubtraction ? `${result} + ${offset} = ${product}` : `${result} − ${offset} = ${product}`, `${product} ÷ ${factor} = □`];
      }
      ...
      else if (structure === "missing-divide-add") { const dividend = answer * factor; result = answer + offset; display = `□ ÷ ${factor} + ${offset} = ${result}`; hint = "Undo addition, then undo division."; steps = [`${result} − ${offset} = ${answer}`, `${answer} × ${factor} = □`]; questionAnswer = dividend; questionValues = { a: factor, b: answer, factor, offset, dividend }; }
      else if (structure === "missing-two-step-subtract") {
        const start = answer + offset + factor;
        result = answer;
        display = `${start} − □ − ${factor} = ${result}`;
        hint = "Find the total amount removed.";
        steps = [`${start} − ${result} = ${start - result}`, `${start - result} − ${factor} = □`];
        questionAnswer = offset;
        questionValues = { a: start, b: factor, factor, offset };
      }
      ...
      return makeQuestion(this, {
        difficulty: clamp(target, 76, 99), display, answer: questionAnswer,
        values: { ...questionValues, result }, structure, hint, steps,
        visual: visual.relationship(steps[0], steps[1], "then", "Undo one step at a time"),
        model: { title: "Reverse both operations", display, lines: [steps[0], steps[1].replace("□", format(questionAnswer))], answer: format(questionAnswer) },
      });
    },
  }),
  define({
  ...
  define({
    ...
    generate({ rng, target }) {
      const structure = pick(rng, this.structures);
      let denominator = pick(rng, target < 45 ? [2, 4] : [3, 4, 5, 8]);
      if (structure === "fraction-non-unit-name" && denominator === 2) denominator = 4;
      const numerator = structure === "fraction-unit-name"
        ? 1
        : structure === "fraction-non-unit-name"
          ? integer(rng, 2, denominator - 1)
          : target < 35 ? 1 : integer(rng, 1, denominator - 1);
      ...
      if (structure === "fraction-groups-count") {
        ...
        return makeQuestion(this, {
          difficulty: clamp(target, 30, 65), type: "choice",
          instruction: `${numerator} of ${denominator} equal groups are indigo. What fraction?`,
          display: "Choose the fraction", answer, answerType: "fraction",
          choices: choices(rng, answer, [fraction(selected, total + unit), fraction(denominator - numerator, denominator), fraction(numerator, total)]),
          values: { numerator, denominator, total }, structure,
          promptVisual: visual.bar(denominator, numerator, null, `${numerator} of ${denominator} equal groups selected`),
          visual: strip, hint: "Count all equal groups, then the selected groups.",
        });
      }
  ...
  define({
    ...
    generate({ rng, target }) {
      ...
      if (structure === "fraction-missing-whole") return makeQuestion(this, {
        ...common,
        display: `${fractionToken(1, denominator)} of □ = ${unit}`,
        answer: quantity,
        values: { a: quantity, numerator: 1, denominator },
        visual: visual.bar(denominator, 1, null, "Build the unknown whole"),
        retrievalKey: `fraction:1/${denominator}`,
        hint: `The whole has ${denominator} equal parts of ${unit}.`,
        steps: [`${unit} × ${denominator} = □`],
        model: { title: "Find the whole", display: `${fractionToken(1, denominator)} of □ = ${unit}`, lines: [`${unit} × ${denominator} = ${quantity}`], answer: String(quantity) },
      });
  ...
  define({
    ...
    generate({ rng, target }) {
      ...
      if (structure === "fraction-equivalent-to-half") {
        const d = pick(rng, [4, 6, 8, 10, 12]);
        return makeQuestion(this, {
          ...common,
          display: `[[1/2]] = [[□/${d}]]`,
          answer: d / 2,
          values: { numerator: 1, denominator: 2, equivalentNumerator: d / 2, equivalentDenominator: d },
          hint: "Half the denominator gives the numerator for one half.",
          visual: visual.fractionStrip(d / 2, d),
        });
      }
  ...
  define({
    ...
    generate({ rng, target }) {
      ...
      let hundredth = hundredths ? integer(rng, 1, 8) : 0;
      if (hundredths && hundredth === tenths) hundredth = (hundredth % 8) + 1;
      ...
      const placeVisual = whole > 0
        ? visual.relationship(
          `${whole} ${whole === 1 ? "one" : "ones"}`,
          `${tenths} tenths${hundredths ? ` + ${hundredth} hundredths` : ""}`,
          "and",
          "Build the decimal",
        )
        : hundredths ? visual.hundredGrid(tenths * 10 + hundredth) : visual.bar(10, tenths);
      const common = { difficulty: clamp(target, 55, 85), values: { a: value }, structure, visual: placeVisual };
      if (structure === "decimal-digit-value") return makeQuestion(this, { ...common, instruction: `What is the value of the ${hundredths ? "hundredths" : "tenths"} digit ${hundredths ? hundredth : tenths}?`, display: String(value), answer: hundredths ? hundredth / 100 : tenths / 10, answerType: "decimal", hint: `Find the ${hundredths ? "hundredths" : "tenths"} place.` });
      ...
      const gridNumerator = tenths * 10 + hundredth;
      const gridValue = Number((gridNumerator / 100).toFixed(2));
      const gridVisual = visual.hundredGrid(gridNumerator);
      return makeQuestion(this, { ...common, type: "choice", instruction: "Which decimal is shown?", display: "Read the hundred grid", answer: gridValue, answerType: "decimal", choices: choices(rng, gridValue, [tenths / 10, hundredth / 100, (tenths + hundredth) / 10]), values: { a: gridValue, numerator: gridNumerator, denominator: 100 }, promptVisual: gridVisual, visual: gridVisual, hint: "Each small square is one hundredth." });
    },
  }),
  define({
  ...
  define({
    ...
    generate({ rng, target }) {
      ...
      if (structure === "decimal-add-tenths") {
        const a = Number((integer(rng, 2, 18) / 10).toFixed(1));
        const b = Number((integer(rng, 1, 6) / 10).toFixed(1));
        return makeQuestion(this, {
          difficulty: clamp(target, 58, 88),
          display: `${a} + ${b}`,
          answer: Number((a + b).toFixed(1)),
          answerType: "decimal",
          values: { a, b },
          structure,
          hint: "Add tenths to tenths.",
          visual: visual.relationship(`${a} = ${Math.round(a * 10)} tenths`, `${b} = ${Math.round(b * 10)} tenths`, "add", "Express both addends in tenths"),
        });
      }
  ...
  define({
    ...
    generate({ rng, target }) {
      ...
      return makeQuestion(this, { difficulty: clamp(target, 32, 93), display: `${values.slice(0, 3).map(format).join(", ")}, □`, answer, answerType: Number.isInteger(answer) ? "whole" : "decimal", values: { a: values[0] }, structure, hint, visual: visual.numberLine(Math.min(...values), Math.max(...values), values, 4, values.length - 1) });
    },
  }),
  define({
  ...
  define({
    ...
    generate({ rng, target }) {
      const structure = pick(rng, this.structures);
      let display; let estimate; let distractors; let hint; let steps; let values;
      if (structure === "estimate-addition") { const a = integer(rng, 250, 880); const b = integer(rng, 220, 890); const roundedA = Math.round(a / 100) * 100; const roundedB = Math.round(b / 100) * 100; estimate = roundedA + roundedB; display = `${a} + ${b}`; distractors = [estimate - 100, estimate + 100, estimate + 300]; hint = "Round each addend to the nearest 100."; steps = [`${a} ≈ ${roundedA}`, `${b} ≈ ${roundedB}`, `${roundedA} + ${roundedB} = ${estimate}`]; values = { a, b, roundedA, roundedB }; }
      else if (structure === "estimate-subtraction") { const a = integer(rng, 700, 1900); const b = integer(rng, 210, a - 200); const roundedA = Math.round(a / 100) * 100; const roundedB = Math.round(b / 100) * 100; estimate = roundedA - roundedB; display = `${format(a)} − ${format(b)}`; distractors = [estimate - 100, estimate + 100, estimate + 300]; hint = "Round both numbers to friendly hundreds."; steps = [`${format(a)} ≈ ${format(roundedA)}`, `${format(b)} ≈ ${format(roundedB)}`, `${format(roundedA)} − ${format(roundedB)} = ${format(estimate)}`]; values = { a, b, roundedA, roundedB }; }
      else if (structure === "estimate-product") { const a = pick(rng, [19, 21, 29, 31, 49, 51]); const b = pick(rng, [9, 11, 19, 21]); const roundedA = Math.round(a / 10) * 10; const roundedB = Math.round(b / 10) * 10; estimate = roundedA * roundedB; display = `${a} × ${b}`; distractors = [estimate / 2, estimate * 2, estimate + 100]; hint = "Round both factors to friendly tens."; steps = [`${a} ≈ ${roundedA}`, `${b} ≈ ${roundedB}`, `${roundedA} × ${roundedB} = ${estimate}`]; values = { a, b, roundedA, roundedB }; }
      else if (structure === "estimate-division") { const divisor = pick(rng, [4, 5, 8, 10]); const q = pick(rng, [20, 30, 40, 50, 60]); const exact = divisor * q + pick(rng, [-2, -1, 1, 2]); const nearby = divisor * q; estimate = q; display = `${exact} ÷ ${divisor}`; distractors = [q - 10, q + 10, q * 2]; hint = `Use a nearby multiple of ${divisor}.`; steps = [`${exact} ≈ ${nearby}`, `${nearby} ÷ ${divisor} = ${q}`]; values = { a: exact, b: divisor, nearby }; }
      else { const a = integer(rng, 350, 650); const b = integer(rng, 350, 650); const exact = a + b; estimate = Math.round(exact / 100) * 100; display = `${a} + ${b}`; distractors = [estimate - 200, estimate + 200, estimate + 500]; hint = "Use magnitude: hundreds first."; steps = [`${a} + ${b} = ${exact}`, `${exact} ≈ ${estimate} to the nearest 100`]; values = { a, b, exact }; }
      return makeQuestion(this, { difficulty: clamp(target, 58, 95), type: "choice", instruction: "Which is the best estimate?", display, answer: format(estimate), choices: choices(rng, format(estimate), distractors.map(format)), values, structure, hint, steps, visual: visual.relationship(display, `≈ ${format(estimate)}`, "rounds to") });
    },
  }),
  define({
    ...
    generate({ rng, target }) {
      ...
      if (structure === "reasonable-addition") { const a = integer(rng, 1200, 4800); const b = integer(rng, 1200, 4800); const exact = a + b; const offset = pick(rng, isCorrect ? [0, 0, 0, 0] : [2000, 5000]); proposed = exact + offset; display = `${format(a)} + ${format(b)} = ${format(proposed)}`; hint = "Estimate the total in thousands, then check."; }
      else if (structure === "reasonable-subtraction") { const a = integer(rng, 3000, 9000); const b = integer(rng, 1000, a - 500); const exact = a - b; const offset = isCorrect ? pick(rng, [0, 0, 0]) : a; proposed = exact + offset; display = `${format(a)} − ${format(b)} = ${format(proposed)}`; hint = "A subtraction result should fit the size of the starting number."; }
      else if (structure === "reasonable-product") { const a = integer(rng, 20, 80); const b = integer(rng, 3, 9); const exact = a * b; const offset = isCorrect ? pick(rng, [0, 0, 0]) : exact * 9; proposed = exact + offset; display = `${a} × ${b} = ${format(proposed)}`; hint = "Round the larger factor to a nearby ten, then check."; }
      else if (structure === "reasonable-division") { const divisor = pick(rng, [4, 5, 8]); const q = integer(rng, 10, 50); const dividend = divisor * q; proposed = isCorrect ? q : q * divisor; display = `${dividend} ÷ ${divisor} = ${proposed}`; hint = "Multiply the proposed quotient by the divisor."; }
      else { const a = integer(rng, 1000, 9000); proposed = isCorrect ? a + 100 : a + 1000; display = `100 more than ${format(a)} is ${format(proposed)}`; hint = "Only the hundreds should change."; }
      return makeQuestion(this, { difficulty: clamp(target, 70, 98), type: "choice", instruction: "Is the proposed answer correct?", display, answer: isCorrect ? "Yes" : "No", choices: ["Yes", "No"], values: {}, structure, hint, visual: visual.relationship("estimate", "proposed answer", "check") });
    },
  }),
  define({
    ...
    generate({ rng, target }) {
      const structure = pick(rng, this.structures);
      let lines; let answer; let options; let hint; let errorLine; let correctedLines;
      if (structure === "error-addition-partition") { lines = ["398 + 245", "398 + 200 = 598", "598 + 40 = 638", "638 + 5 = 643"]; answer = "Correct"; options = ["Correct", "The tens step is wrong", "The ones step is wrong"]; hint = "Check each partial addition."; errorLine = -1; correctedLines = lines; }
      else if (structure === "error-subtraction-compensation") { lines = ["502 − 298", "502 − 300 = 202", "202 − 2 = 200"]; answer = "The adjustment is wrong"; options = ["Correct", "The adjustment is wrong", "The first subtraction is wrong"]; hint = "Subtracting 300 removed 2 too much."; errorLine = 2; correctedLines = ["502 − 300 = 202", "202 + 2 = 204", "So 502 − 298 = 204"]; }
      else if (structure === "error-multiplication-partial") { lines = ["48 × 6", "40 × 6 = 240", "8 × 6 = 48", "240 + 48 = 278"]; answer = "The final addition is wrong"; options = ["Correct", "The 40 × 6 fact is wrong", "The final addition is wrong"]; hint = "Check the partial products, then their sum."; errorLine = 3; correctedLines = ["40 × 6 = 240", "8 × 6 = 48", "240 + 48 = 288"]; }
      else if (structure === "error-division-inverse") { lines = ["420 ÷ 7", "42 ÷ 7 = 6", "So 420 ÷ 7 = 6"]; answer = "The place-value scale is missing"; options = ["Correct", "The known fact is wrong", "The place-value scale is missing"]; hint = "420 is ten times 42."; errorLine = 2; correctedLines = ["42 ÷ 7 = 6", "420 is 10 times 42", "So 420 ÷ 7 = 60"]; }
      else { lines = ["[[3/4]] of 20", "20 ÷ 3 = 6 remainder 2", "6 × 4 = 24"]; answer = "Divide by the denominator first"; options = ["Correct", "Divide by the denominator first", "Multiply by 20 first"]; hint = "The denominator tells how many equal parts."; errorLine = 1; correctedLines = ["20 ÷ 4 = 5", "5 × 3 = 15", "So [[3/4]] of 20 = 15"]; }
      return makeQuestion(this, {
        difficulty: clamp(target, 72, 99),
        type: "choice",
        instruction: "Check the method",
        display: "Correct or find the first error",
        answer,
        choices: shuffle(rng, options),
        values: {},
        structure,
        promptVisual: visual.worked(lines, errorLine),
        visual: visual.worked(lines, errorLine),
        hint,
        model: { title: answer === "Correct" ? "Confirm each step" : "Correct the method", display: "Correct or find the first error", lines: correctedLines, answer },
      });
    },
  }),
  define({
