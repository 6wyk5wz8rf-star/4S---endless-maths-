...
export const BUILD2_FAMILIES = [
  ...
  define({
    ...
    generate({ rng, target, challenge = target }) {
      ...
      else if (structure === "subtract-compensation") { a = integer(rng, 120, 700); b = pick(rng, [98, 198, 299]); hint = `Subtract ${b + (b % 100 === 99 ? 1 : 2)}, then adjust.`; itemVisual = visual.relationship(`${a} − ${Math.round(b / 100) * 100}`, "+ adjustment", "then"); strategy = "compensation"; }
  ...
  define({
    ...
    generate({ rng, target, challenge = target }) {
      ...
      if (structure === "repeated-addition") return makeQuestion(this, { ...common, type: "choice", instruction: `Which matches ${a} × ${b}?`, display: `${a} × ${b}`, answer: Array.from({ length: b }, () => a).join(" + "), choices: choices(rng, Array.from({ length: b }, () => a).join(" + "), [Array.from({ length: a }, () => b + 1).join(" + "), `${a} + ${b}`, `${product} + ${b}`]), hint: "Multiplication describes equal groups." });
  ...
  define({
    ...
    generate({ rng, target }) {
      ...
      if (structure === "compose-number") return makeQuestion(this, { ...common, instruction: "What number is this?", display: `${digits[0]} thousands + ${digits[1]} hundreds + ${digits[2]} tens + ${digits[3]} ones`, answer: value, hint: "Place each digit in its column." });
  ...
  define({
    ...
    generate({ rng, target }) {
      ...
      if (structure === "line-midpoint") return makeQuestion(this, { ...common, instruction: "What is halfway?", display: `${format(min)} — ${format(max)}`, answer: min + unit * 2, promptVisual: visual.numberLine(min, max, [min + unit * 2], 5, 0), visual: visual.numberLine(min, max, [min + unit * 2], 5), hint: "Halve the distance between the endpoints." });
      if (structure === "line-missing-interval") return makeQuestion(this, { ...common, display: `${format(min)}, ${format(min + unit)}, □, ${format(min + unit * 3)}`, answer: min + unit * 2, promptVisual: visual.numberLine(min, max, [min, min + unit, min + unit * 3], 5, 2), visual: visual.numberLine(min, max, [min, min + unit, min + unit * 3], 5, 2), hint: `Each interval is ${format(unit)}.` });
      if (structure === "line-estimate-position") { const marker = min + integer(rng, 1, 3) * unit; return makeQuestion(this, { ...common, type: "choice", instruction: "Which value is marked?", display: "Read the line", answer: format(marker), choices: choices(rng, format(marker), [format(marker - unit), format(marker + unit), format(marker + unit / 2)]), promptVisual: visual.numberLine(min, max, [marker], 5), visual: visual.numberLine(min, max, [marker], 5), hint: "Work out the value of one equal interval." }); }
      ...
      const marker = min + unit * integer(rng, 1, 3);
      return makeQuestion(this, { ...common, display: `Point A = □`, answer: marker, promptVisual: visual.numberLine(min, max, [marker], 5), visual: visual.numberLine(min, max, [marker], 5), hint: `The line increases by ${format(unit)} each interval.` });
    },
  }),
  define({
  ...
  define({
    ...
    generate({ rng, target }) {
      ...
      if (structure === "subtraction-count-up") {
        ...
        hint = "The numbers are close. Count up to find the difference."; steps = [`Count from ${format(b)} to a nearby landmark.`, `Continue to ${format(a)} and combine the jumps.`]; itemVisual = visual.numberLine(b, a, [b, Math.ceil(b / 100) * 100, a], 5);
  ...
  define({
    ...
    generate({ rng, target }) {
      ...
      const structure = pick(rng, this.structures);
      const answer = fraction(numerator, denominator);
      const line = visual.numberLine(0, 1, [numerator / denominator], denominator + 1, structure.includes("missing") ? numerator : null, "Fractions from 0 to 1");
      if (structure === "fraction-line-halfway") return makeQuestion(this, { difficulty: clamp(target, 64, 92), display: `Halfway between 0 and ${fractionToken(2, denominator)}`, answer: fraction(1, denominator), answerType: "fraction", values: { numerator: 1, denominator }, structure, promptVisual: visual.numberLine(0, 2 / denominator, [1 / denominator], 3, 1), visual: line, hint: "Halve the distance on the line." });
  ...
  define({
    ...
    generate({ rng, target }) {
      ...
      const structure = pick(rng, hundredths ? this.structures : this.structures.slice(0, 4));
      const line = visual.numberLine(start, max, [marker], 6, structure.includes("missing") ? markerIndex : null, "Decimals on a number line");
      if (structure === "decimal-line-midpoint") { const end = Number((start + step * 4).toFixed(2)); const middle = Number((start + step * 2).toFixed(2)); return makeQuestion(this, { difficulty: clamp(target, 61, 90), instruction: "What is halfway?", display: `${start} — ${end}`, answer: middle, answerType: "decimal", values: { a: start, b: end }, structure, promptVisual: visual.numberLine(start, end, [middle], 5, 2), visual: line, hint: "Find half the interval." }); }
  ...
  define({
    ...
    generate({ rng, target }) {
      ...
      if (structure === "decimal-equivalent-zeros") { const a = Number((integer(rng, 1, 9) / 10).toFixed(1)); return makeQuestion(this, { difficulty: clamp(target, 56, 86), type: "choice", instruction: "True or false?", display: `${a} = ${a.toFixed(2)}`, answer: "True", choices: ["True", "False"], values: { a }, structure, hint: "A trailing zero does not change the value." }); }
      const values = [0.4, 0.35, 0.7]; const sorted = [...values].sort((a, b) => a - b); const answer = sorted.join(" < "); return makeQuestion(this, { difficulty: clamp(target, 68, 92), type: "choice", instruction: "Which order is smallest to largest?", display: values.join("   "), answer, choices: choices(rng, answer, [[...sorted].reverse().join(" < "), [sorted[1], sorted[0], sorted[2]].join(" < ")], 3), values: {}, structure, hint: "Compare tenths, then hundredths." });
    },
  }),
  define({
