/**
 * 4S Arithmetic classroom data and evidence core.
 *
...
const APPLICATION_DATA_FIELDS = new Set(["preferences", "learningState", "generatorHistory", "activeSession", "lastSession"]);
...
function sanitiseJsonValue(value, depth = 0) {
  if (depth > 32) return undefined;
  if (value === null || typeof value === "string" || typeof value === "boolean") return value;
  if (typeof value === "number") return Number.isFinite(value) ? value : null;
  if (Array.isArray(value)) return value.slice(0, 100_000).map((item) => sanitiseJsonValue(item, depth + 1)).filter((item) => item !== undefined);
  if (!value || typeof value !== "object") return undefined;
  const result = {};
  for (const [key, item] of Object.entries(value)) {
    if (UNSAFE_OBJECT_KEYS.has(key)) continue;
    const safe = sanitiseJsonValue(item, depth + 1);
    if (safe !== undefined) result[key] = safe;
  }
  return result;
}

function sanitiseApplicationData(value) {
  const source = asObject(value);
  const result = {};
  for (const field of APPLICATION_DATA_FIELDS) {
    if (!Object.prototype.hasOwnProperty.call(source, field)) continue;
    const safe = sanitiseJsonValue(source[field]);
    if (safe !== undefined) result[field] = safe;
  }
  return result;
}

function validateJsonTree(value, path, errors, depth = 0, seen = new WeakSet()) {
  if (depth > 32) {
  ...
  if (!value || typeof value !== "object") {
    errors.push(`${path} contains a value that cannot be restored.`);
    return;
  }
  if (seen.has(value)) {
  ...
  if (Array.isArray(value)) {
    if (value.length > 100_000) errors.push(`${path} contains too many items.`);
    value.forEach((item, index) => validateJsonTree(item, `${path}[${index}]`, errors, depth + 1, seen));
  } else {
    for (const [key, item] of Object.entries(value)) {
      if (UNSAFE_OBJECT_KEYS.has(key)) errors.push(`${path}.${key} is not permitted.`);
      else validateJsonTree(item, `${path}.${key}`, errors, depth + 1, seen);
    }
  }
  seen.delete(value);
}
...
export const DEFAULT_CLASSROOM_PRESETS = Object.freeze([
  ...
  preset("year-4-core", "Year 4 Core", "Year 4 arithmetic and place value.", {
  ...
  preset("deep-challenge", "Deep Challenge", "Harder structure, equivalence and efficient calculation.", {
...
export function createClassroomBackup(state, options = {}) {
  ...
  if (Object.prototype.hasOwnProperty.call(options, "applicationData")) {
    packageValue.applicationData = sanitiseApplicationData(options.applicationData);
  }
  return packageValue;
}
...
export function validateClassroomPackage(value, expectedType = null) {
  ...
  if (packageValue.type === CLASSROOM_PACKAGE_TYPE && Object.prototype.hasOwnProperty.call(packageValue, "applicationData")) {
    if (!packageValue.applicationData || typeof packageValue.applicationData !== "object" || Array.isArray(packageValue.applicationData)) {
      errors.push("Backup application data is not valid.");
    } else {
      for (const field of Object.keys(packageValue.applicationData)) {
        if (!APPLICATION_DATA_FIELDS.has(field)) errors.push(`Unknown application data field: ${field}.`);
      }
      for (const field of ["preferences", "learningState"]) {
        const fieldValue = packageValue.applicationData[field];
        if (fieldValue !== undefined && (!fieldValue || typeof fieldValue !== "object" || Array.isArray(fieldValue))) {
          errors.push(`Application data ${field} must be an object.`);
        }
      }
      for (const field of ["activeSession", "lastSession"]) {
        const fieldValue = packageValue.applicationData[field];
        if (fieldValue !== undefined && fieldValue !== null && (!fieldValue || typeof fieldValue !== "object" || Array.isArray(fieldValue))) {
          errors.push(`Application data ${field} must be an object or null.`);
        }
      }
      validateJsonTree(packageValue.applicationData, "applicationData", errors);
      if (!errors.length) applicationData = sanitiseApplicationData(packageValue.applicationData);
    }
  }
  return { valid: errors.length === 0, errors, type: packageValue.type, data, applicationData };
}
...
function latestTimestamp(value, fields = ["updatedAt", "completedAt", "timestamp", "createdAt", "startedAt"]) {
...
function deterministicWinner(left, right, fields) {
  const leftTime = latestTimestamp(left, fields);
  const rightTime = latestTimestamp(right, fields);
  if (leftTime !== rightTime) return leftTime > rightTime ? left : right;
  return canonicalJson(left).localeCompare(canonicalJson(right)) >= 0 ? left : right;
}

function deterministicUnionById(leftValues, rightValues, normaliser, combine = deterministicWinner) {
  const values = [...leftValues, ...rightValues].map((item, index) => normaliser(item, index));
  const grouped = new Map();
  for (const item of values) {
    const existing = grouped.get(item.id);
    grouped.set(item.id, existing ? combine(existing, item) : item);
  }
  return [...grouped.values()].sort((left, right) => left.id.localeCompare(right.id));
}

function newestIso(...values) {
  const valid = values.map((value) => isoDate(value, null)).filter(Boolean).sort();
...
/**
 * Reconcile independently-written classroom snapshots without last-writer data
 * loss. The operation is commutative and idempotent: entity/evidence IDs are
 * unioned, timestamped conflicts choose the newest record, ties use canonical
 * JSON ordering, and settings come from the snapshot with the newest meta time.
 */
export function mergeClassroomStates(leftValue, rightValue, options = {}) {
  const left = migrateClassroomState(leftValue, options);
  const right = migrateClassroomState(rightValue, options);
  const profiles = deterministicUnionById(left.profiles, right.profiles, normaliseProfile, (first, second) => {
    const winner = deterministicWinner(first, second);
    return { ...winner, groupIds: [...new Set([...first.groupIds, ...second.groupIds])].sort() };
  });
  const groups = deterministicUnionById(left.groups, right.groups, normaliseGroup, (first, second) => {
    const winner = deterministicWinner(first, second);
    return { ...winner, profileIds: [...new Set([...first.profileIds, ...second.profileIds])].sort() };
  });
  const presets = deterministicUnionById(left.presets, right.presets, normalisePreset);
  const sessions = deterministicUnionById(left.sessions, right.sessions, normaliseSession, (first, second) => {
    const winner = deterministicWinner(first, second);
    const responses = deterministicUnionById(first.responses, second.responses, (response, index) => ({
      ...response,
      id: cleanId(response.eventId || response.itemId, `response-${index}`),
    }), deterministicWinner).map(({ id: _id, ...response }) => response);
    return {
      ...winner,
      profileIds: [...new Set([...first.profileIds, ...second.profileIds])].sort(),
      responses,
    };
  }).slice(-200);
  const notes = deterministicUnionById(left.notes, right.notes, normaliseNote).slice(-2_000);
  const events = normaliseEvents(deterministicUnionById(left.events, right.events, normaliseEvent, (first, second) => deterministicWinner(first, second, ["timestamp"])), options);
  const settingsSource = deterministicWinner(
    { updatedAt: left.meta.updatedAt, settings: left.settings },
    { updatedAt: right.meta.updatedAt, settings: right.settings },
    ["updatedAt"],
  );
  const recentSessionIds = [...new Set([...left.recentSessionIds, ...right.recentSessionIds])]
    .sort((first, second) => {
      const firstSession = sessions.find((session) => session.id === first);
      const secondSession = sessions.find((session) => session.id === second);
      return latestTimestamp(firstSession) - latestTimestamp(secondSession) || first.localeCompare(second);
    })
    .slice(-12);
  const builtInOrder = new Map(DEFAULT_CLASSROOM_PRESETS.map((item, index) => [item.id, index]));
  presets.sort((first, second) => {
    const firstIndex = builtInOrder.get(first.id);
    const secondIndex = builtInOrder.get(second.id);
  ...
  return migrateClassroomState({
    schemaVersion: CLASSROOM_SCHEMA_VERSION,
    applicationVersion: [left.applicationVersion, right.applicationVersion].sort().at(-1),
    profiles,
    groups,
    presets,
    sessions,
    ...
    meta: {
      createdAt: oldestIso(left.meta.createdAt, right.meta.createdAt),
      updatedAt: newestIso(left.meta.updatedAt, right.meta.updatedAt),
      lastCompactedAt: newestIso(left.meta.lastCompactedAt, right.meta.lastCompactedAt),
    },
  }, options);
}
...
export function previewBackupRestore(state, packageValue, options = {}) {
  ...
  const applicationDataAction = validation.applicationData === null ? "none" : replace ? "restore" : "preserve-current";
  const applicationDataNote = applicationDataAction === "preserve-current"
    ? " Current pupil preferences, learning memory and unfinished practice will be kept; application data from the backup is only restored in Replace mode."
    : applicationDataAction === "restore"
      ? " Pupil preferences, learning memory and unfinished practice in this backup will also be restored."
      : "";
  return {
    valid: true,
    errors: [],
    ...
    hasApplicationData: validation.applicationData !== null,
    applicationDataAction,
    warning: `${replace ? "Current classroom data will be replaced after confirmation." : "Existing data will be preserved and compatible records added."}${applicationDataNote}`,
  };
}
...
export function restoreClassroomBackup(state, packageValue, options = {}) {
  ...
  if (preview.mode === "replace") {
    return {
      restored: true,
      state: { ...incoming, settings: { ...incoming.settings, teacherPinHash: "", teacherAccessConfigured: false } },
      applicationData: Object.prototype.hasOwnProperty.call(packageValue, "applicationData") ? sanitiseApplicationData(packageValue.applicationData) : null,
      report: preview,
    };
  }
  const eventMerge = mergeImportedEventsPreservingCurrent(current.events, incoming.events, options);
  ...
  return { restored: true, state: next, applicationData: null, report: preview };
}
