...
export type TeacherToolsProps = {
  ...
  onClose: () => void;
  onLock: () => void;
  onLaunch: (config: TeacherSessionConfig, profileIds?: string[]) => void;
...
const CURRICULUM_MAP = [
  ...
  {
    title: "Year 4 core",
    note: "The broad centre of the fluency continuum.",
    items: [
...
export const CORE_TEACHER_PRESETS: TeacherPreset[] = [
  ...
  { id: "year-4-core", name: "Year 4 Core", note: "Broad arithmetic and place-value fluency.", builtIn: true, config: presetConfig({ challenge: { kind: "range", min: 46, max: 68 }, support: "adaptive", length: { kind: "questions", value: 15 }, representationFrequency: "balanced", pupilControls: PRESET_CONTROLS }) },
...
export function TeacherTools(props: TeacherToolsProps) {
  ...
  function startPreset(preset: TeacherPreset) {
    props.onLaunch(preset.config, selectedProfiles.length ? selectedProfiles : undefined);
  }
      ...
      <main id="teacher-main" className="teacher-main" tabIndex={-1}>
        {destination === "start" && (
          <div className="teacher-page teacher-start-page">
            ...
            {activeProfiles.length > 0 && <section className="teacher-participants" aria-labelledby="participants-heading"><div><h2 id="participants-heading">Who is practising?</h2><p>{selectedProfiles.length ? `${selectedProfiles.length} selected` : "Whole class or guest"}</p>{selectedProfiles.length > 1 && <small>Group practice does not create individual pupil evidence.</small>}</div><div className="teacher-chip-list"><button type="button" className={selectedProfiles.length === 0 ? "is-selected" : ""} onClick={() => setSelectedProfiles([])}>Whole class / guest</button>{props.groups.map((group) => { const selected = group.profileIds.length > 0 && group.profileIds.length === selectedProfiles.length && group.profileIds.every((id) => selectedProfiles.includes(id)); return <button type="button" className={selected ? "is-selected" : ""} key={group.id} onClick={() => setSelectedProfiles(selected ? [] : group.profileIds)}>{group.name}</button>; })}<select aria-label="Choose one pupil profile" value={selectedProfiles.length === 1 ? selectedProfiles[0] : ""} onChange={(event) => setSelectedProfiles(event.target.value ? [event.target.value] : [])}><option value="">One pupil…</option>{activeProfiles.sort((a, b) => a.displayName.localeCompare(b.displayName)).map((profile) => <option key={profile.id} value={profile.id}>{profile.displayName}</option>)}</select></div></section>}
            ...
            <div className="teacher-two-column">
              ...
              <section className="teacher-section"><div className="teacher-section-heading"><div><p>Recently used</p><h2>Recent sessions</h2></div></div>{props.recentSessions.slice(0, 5).length ? props.recentSessions.slice(0, 5).map((session) => <article className="teacher-recent-row" key={session.id}><button type="button" onClick={() => props.onLaunch(session.config, selectedProfiles.length ? selectedProfiles : undefined)}><strong>{session.label}</strong><small>{formatDate(session.usedAt)} · {configSummary(session.config)}</small></button><button type="button" className="teacher-text-button" onClick={() => { setConfig(session.config); setEditingPresetId(null); changeDestination("build"); }}>Edit</button></article>) : <EmptyState>Your most recent session choices will appear here.</EmptyState>}</section>
            </div>
          </div>
        )}
        ...
        {destination === "build" && (
          <div className="teacher-page teacher-build-page">
            ...
            <div className="teacher-builder">
              <form className="teacher-builder-form" onSubmit={(event) => { event.preventDefault(); props.onLaunch(config, selectedProfiles.length ? selectedProfiles : undefined); }}>
